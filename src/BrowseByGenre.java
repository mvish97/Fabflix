...
public class BrowseByGenre extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String link = BrowseByGenre.processRequest(request);
		...
		try {
         ...
         // Pooling Code
            ...
            String query = "select id, title, year, dir, genres, actors, r.rating, ids "
            		...
            		+ "group_concat(distinct s.id) as ids from movies m, "
            		+ "genres_in_movies gim, genres g, stars_in_movies sim, stars s where m.id = gim.movieID and "
            		+ "gim.genreId = g.id and sim.movieID = m.id and sim.starId = s.id group by m.id) "
